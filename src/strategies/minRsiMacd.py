...
class MinRsiMacd(BaseStrategy):
    ...
    def next(self):
        ...
        for i in range(len(self.datas)):
            self.score[i] = 1 / self.rsi[i][0] * 100
            if self.macd[i].macd[0] > self.macd[i].signal[0]:
                self.score[i] += 20
            if self.macd[i].macd[0] < self.macd[i].signal[0] and self.macd[i].macd[-1] > self.macd[i].signal[-1]:
                self.score[i] -= 30
...
if __name__ == '__main__':
    ...
    # Add a strategy
    cerebro.addstrategy(MinRsiMacd)
